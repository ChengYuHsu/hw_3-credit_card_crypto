...
module DoubleTranspositionCipher
  ...
  def self.encrypt(document, key)
    ...
    ((doc_str.length**0.5).to_i..doc_str.length).each do |num|
      ...
      return doc_str.chars.each_with_index.map do |dummy, index|
  ...
  def self.decrypt(ciphertext, key)
    ...
    ((cip_str.length**0.5).to_i..cip_str.length).each do |num|
      ...
      mapping = self.gen_mapping(cip_str.length, num, key, reversed = true)
      return cip_str.chars.each_with_index.map do |dummy, index|
